...
let keyIndex = 0
function nextKey() { const k = apiKeys[keyIndex % apiKeys.length]; keyIndex++; return k }
...
async function generatePlan(projectId, questionnaire, apiKey) {
...
async function main() {
  for (const id of failedIds) {
    ...
    const key = nextKey()
    try {
      console.log(`[${id}] Generating (key ${(keyIndex - 1) % apiKeys.length + 1})...`)
      const content = await generatePlan(id, q, key)
