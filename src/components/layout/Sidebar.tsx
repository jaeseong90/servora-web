    ...
    <div className="flex flex-col h-full py-8 justify-between">
      ...
      <div className={`flex flex-col gap-6 ${collapsed ? 'px-4 items-center' : 'px-6'}`}>
        <div className={`flex items-center ${collapsed ? 'flex-col gap-4' : 'justify-between'}`}>
          ...
          <button
            onClick={() => {
              if (onMobileClose) { onMobileClose() } else { setCollapsed(!collapsed) }
            }}
          ...
          >
            <span className="material-symbols-outlined">
              {onMobileClose ? 'close' : collapsed ? 'menu' : 'menu_open'}
            </span>
          </button>
        </div>
        <div className="flex justify-center">
          <button
            onClick={() => { onNewService(); onMobileClose?.() }}
          ...
          >
            ...
            {!collapsed && <span className="text-[13px] tracking-wide">New Service</span>}
          </button>
        </div>
      </div>
      ...
      <div className="flex-1 mt-10 space-y-8 overflow-y-auto custom-scrollbar">
        ...
        <nav className="space-y-1">
          ...
          >
            ...
            {!collapsed && <span className="text-sm font-medium">Select Service</span>}
          </div>
        </nav>
        ...
        <nav className="space-y-1">
          ...
          {['Planning', 'Design', 'MVP'].map((label) => (
            <div key={label} className={`flex items-center gap-4 text-on-surface-variant py-2.5 px-6 opacity-30 cursor-not-allowed ${collapsed ? 'justify-center px-0' : ''}`}>
          ...
          {['Service', 'Operation'].map((label) => (
            <div key={label} className={`flex items-center justify-between py-2.5 px-6 opacity-40 cursor-not-allowed ${collapsed ? 'justify-center px-0' : ''}`}>
